...
function getTypeIcon(type) {
  ...
  switch (type) {
    ...
    case 'Application':
      return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${stroke}" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg>`;
    case 'Video':
...
function renderAdminTable(searchQuery = '') {
  ...
  if (state.currentUser.role === 'teacher') {
    ...
    document.getElementById('table-media-title').innerText = `รายการสื่อการสอนของคุณ (${filtered.length} รายการ)`;
  } else {
    document.getElementById('table-media-title').innerText = `รายการสื่อการสอนทั้งหมดในระบบ (${filtered.length} รายการ)`;
  }
  ...
  document.getElementById('admin-media-count').innerText = filtered.length;
